...
function CityInfoCtrl:Awake(go)
    ...
    cityInfoBehaviour:AddClick(CityInfoPanel.notBasic,self.OnNotBasic,self)  --basic information
    ...
    cityInfoBehaviour:AddClick(CityInfoPanel.notIndustry,self.OnNotIndustry,self)  --Industry Information
    ...
    cityInfoBehaviour:AddClick(CityInfoPanel.productDown,self.OnDown,self)  --Second level list
    cityInfoBehaviour:AddClick(CityInfoPanel.close,self.OnClose,self)  --Close the second level list

    self.width = CityInfoPanel.titleBg.sizeDelta.x
    self.ownerId = DataManager.GetMyOwnerID()
    self.first = true  --Click on industry information for the first time
    self.rankItem = {}   --Leaderboard example
...
function CityInfoCtrl:OnNotIndustry(go)
    ...
    if go.first then
        DataManager.DetailModelRpcNoRet(go.m_data.insId, 'm_queryIndustryIncome')  --Check industry income
    end
    go.first = false
    local last = go.industryInfoItem[1]:GetLast():GetTitle()
    if last then
...
function CityInfoCtrl:SupplyAndDemand(info,graph,slide,curve,yScale,int)
    ...
    local updataTime = sevenAgo
    local time = {}
    local avgPriceTab = {}       --Number of shelves (supply)
    local purchasesTab = {}     --Purchase volume (demand)
    if createTs >= sevenAgo then
        updataTime = createTs
        for i = 1, 7 do
            time[i] = getFormatUnixTime(updataTime).month .. "." .. getFormatUnixTime(updataTime).day
...
function CityInfoCtrl:OneCurve(info,graph,slide,curve,yScale,int)
    ...
    local updataTime = sevenAgo
    local time = {}
    local avgPriceTab = {}      --Average transaction price
    if createTs >= sevenAgo then
        updataTime = createTs
        for i = 1, 7 do
            time[i] = getFormatUnixTime(updataTime).month .. "." .. getFormatUnixTime(updataTime).day
