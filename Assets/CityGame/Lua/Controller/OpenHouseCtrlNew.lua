...
function OpenHouseCtrlNew:_getComponent(go)
    ...
    self.comCenterText = transform:Find("root/competitiSlider/center/Image/Text/valueText"):GetComponent("Text")  --Half competitive
...
function OpenHouseCtrlNew:_awakeSliderInput()
    self.rentInput.onValueChanged:AddListener(function (str)
        ...
        if finalStr ~= str then
            self.rentInput.text = finalStr  --Limit user decimal input
            return
        end
        local temp = ct.CalculationHouseCompetitivePower(self.guideData.guidePrice, tonumber(str) * 10000, self.guideData.npc)  --Calculate competitiveness
        ...
        OpenHouseCtrlNew.sliderCanChange = false  --When input is input, the slider is disabled
    ...
    EventTriggerMgr.Get(self.competitiSlider.gameObject).onUpdateSelected = function()  --When slider is selected, you can change the value of input
        OpenHouseCtrlNew.sliderCanChange = true
    end
...
function OpenHouseCtrlNew:_language()
    ...
    self.comCenterText13.text = GetLanguage(12345678)  --Half of the competitiveness
    --self.competitivenessText11.text = GetLanguage(43010002)
    --self.competitivenessText12.text = GetLanguage(43010003)
end
--
function OpenHouseCtrlNew:_initData()
    ...
    DataManager.m_ReqHouseGuidPrice(self.m_data.info.id)  --Request competitiveness parameters from the server
    ...
    self.staffNumText.text = self.staffNum  --Employee
    ...
    if self.standardWage == nil then
        ...
        local value = self.staffNum * self.standardWage  --temp modify
...
function OpenHouseCtrlNew:_getStandardWage(data)
    if data.industryWages ~= nil then
        ...
        local value = self.staffNum * data.industryWages  --temp modify
        self.totalText.text = "E"..GetClientPriceString(value)
        self.totalValue = value
    end
end
...
function OpenHouseCtrlNew:_onClickConfirm(ins)
    ...
    if DataManager.GetMoney() < ins.totalValue then
        Event.Brocast("SmallPop", GetLanguage(41010006), 300)  --Fund judgment
        return
    end
    ...
    if ins.m_data.callBackFunc ~= nil then
        ...
        ct.OpenCtrl("OpenBuildingCheckCtrl", data)  --Second confirmation interface
