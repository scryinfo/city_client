---
--- Generated by EmmyLua(https://github.com/EmmyLua)
--- Created by xuyafang.
--- DateTime: 2019/2/27 18:13
---Building secondary menu
MapBuildsPageItem = class('MapBuildsPageItem', MapDetailPageBase)

--initialization
function MapBuildsPageItem:childInit(viewRect)
    self.content = viewRect:Find("scroll/content")
    self.bgImg = viewRect:Find("Image"):GetComponent("Image")
    self.text01 = viewRect:Find("Image/Text"):GetComponent("Text")
end
--Create items
function MapBuildsPageItem:createItems()
    self.items = {}
    for i, value in ipairs(MapBuildInfoConfig) do
        local go = UnityEngine.GameObject.Instantiate(MapPanel.mapBuildsDetailItem)
        go.transform:SetParent(self.content.transform)
        go.transform.localScale = Vector3.one
        local data = {promotionIndex = i, mapSearchType = EMapSearchType.Builds , itemId = value.type}
        local item = MapBuildsDetailItem:new(data, go)
        self.items[i] = item
    end
    self:sortItemPos(self.items)
end
--
function MapBuildsPageItem:initLanguage()
    self.text01.text = GetLanguage(20010005)
    local width = self.text01.preferredWidth + 35
    self.bgImg.rectTransform.sizeDelta = Vector2.New(width, 50)
end
--
function MapBuildsPageItem:sortItemPos(itemTable)
    if itemTable ~= nil and #itemTable ~= 0 then
        local pos = Vector3.zero
        for i, item in ipairs(itemTable) do
            item:setPos(pos)
            pos = pos - Vector3.New(0, 125, 0)
        end
    end
end