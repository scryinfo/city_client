...
--Doc
--[[
Here again briefly talk about the use of tools.
　　First require this script, for example: local mri = require(MemoryReferenceInfo).
　　 Then print a memory snapshot somewhere:

collectgarbage("collect")
mri.m_cMethods.DumpMemorySnapshot("./", "All", -1)

　　 The content of the snapshot file, each line is a reference object information, all information is arranged in descending order of the number of citations, each line is divided into 3 columns by tab, which are: object type/address,
    Reference chain, number of citations. The entire file can be opened using Excel, and it will be automatically divided into 3 columns for easy reading and reordering.

　　 The key part of the file content is the information of the reference chain, for example "function: 0x7f85f8e0e3f0 registry.2[_G].Author.Ask[line:33@file:example.lua] 1"
    This message shows that member "2" of table "registry" (that is, table "_G") refers to table "Author", and table "Author" has a member "Ask" that references "function: 0x7f85f8e0e3f0",
    The location of the function is on line 33 in the file "example.lua", and it is cited once. In this way, you can quickly locate what objects are referenced and how many times they are referenced.

　　 "DumpMemorySnapshot" The last two parameters of this method are "root node object name" and "search root node object", the default values ​​are "registry" and "debug.getregistry()",
    In most cases, you do not need to modify the default values, but when you want to start searching from another root node to narrow the scope, for example, from "_G", you can manually set these two parameters, for example:

- Only dump memory snapshot searched from "_G".
collectgarbage("collect")
mri.m_cMethods.DumpMemorySnapshot("./", "All", -1, "_G", _G)

　　After the entire program runs for a period of time, print another memory snapshot (you can print multiple copies), the next most important job is to compare the snapshot analysis to increase the leakage point.
    In this tool, an interface called "DumpMemorySnapshotComparedFile" is provided to implement this comparison function. Remember not to compare the two snapshots with a file comparison tool (some friends have used this),
    Because the snapshot content is sorted in descending order according to the reference count, the content is different at different times, and the order is also different, so ordinary file comparison tools cannot take effect here. Instructions:

mri.m_cMethods.DumpMemorySnapshotComparedFile("./", "Compared", -1,
"./LuaMemRefInfo-All-[1-Before].txt",
"./LuaMemRefInfo-All-[2-After].txt")

This method will generate a new file that contains the data that appears in the second snapshot but does not appear in the first snapshot. This is the new content.

Regardless of the type of data, if the data is too large after dumping, but you want to view a specific data, you can use a filter to generate a new file, you can choose whether the content generated by the new file contains keywords or excludes keywords ,E.g:

- All content in the output file that contains the keyword "Author". (not case sensitive)
mri.m_cBases.OutputFilteredResult("./LuaMemRefInfo-All-[2-After].txt", "Author", true, true)

--All content in the output file that does not contain the keyword "Author". (not case sensitive)
...
　　 In addition, if you want to see where an object is referenced, you can use the interface "DumpMemorySnapshotSingleObject", for example:

- Output all references to the object "_G.Author".
collectgarbage("collect")
mri.m_cMethods.DumpMemorySnapshotSingleObject("./", "SingleObjRef-Object", -1, "Author", _G.Author)

- Output all quoted strings "yaukeywang".
collectgarbage("collect")
mri.m_cMethods.DumpMemorySnapshotSingleObject("./", "SingleObjRef-String", -1, "Author Name", "yaukeywang")

　　Through the use of the above main methods, you can quickly detect memory leaks, even on mobile phones. For example, when printing, point the save path to the sd card directory. For example, if you use Lua in Unity, you can use:

collectgarbage("collect")
mri.m_cMethods.DumpMemorySnapshot(UnityEngine.Application.persistentPath, "All", -1)

It will output a snapshot file to the sd card directory.

　　 Now there is a new configuration option, generally for example "DumpMemorySnapshot" This method is to specify a save path and additional information, and then the saved file name will be added with the current timestamp every time, which is convenient to distinguish different snapshots according to time,
    It also avoids the need to set and modify the file name frequently, and avoids the snapshots of the same place at different times being constantly overwritten. This timestamp option is turned on by default, and can be turned off by "mri.m_cConfig.m_bAllMemoryRefFileAddTime = false".
    The configuration settings are placed after "require" and before Dump. Several other Dump interfaces also have the option of adding a timestamp to the file name. For details, see the source code.

In addition to the above methods, it also provides some other interfaces that can be used. For more detailed use, please refer to the ReadMe on GitHub and the interface definition instructions in the source code. They are all written in great detail. "Example.lua" also demonstrates commonly used How to use the interface.

　　Finally, the tool has recently been improved, and the output of the string type has been added, so in the search path diagram above, you can add a "string" to the path.
    At the same time, it should be noted that in order to display all the strings on the same line (to facilitate the processing of data by other methods, such as comparing the difference increment, Excel sort statistics, etc.),
    When the string is displayed, all carriage returns and line feeds:'\r','\n' are explicitly replaced with'\\n', please pay attention when reading the data.
