...
function WalletCtrl:Awake(go)
    ...
    self.luaBehaviour:AddClick(self.scanningBtn.gameObject,self._clickScanningBtn,self)   --Scan code is temporarily closed
    ...
    self.luaBehaviour:AddClick(self.QRCodeConfirm.gameObject,self._clickQRCodeConfirmBtn,self)   --Go to CashBoxs
    --self.luaBehaviour:AddClick(self.copyBtn.gameObject,self._clickCopyBtnBtn,self)   --copy

   --Slide interoperability 
    self.trading = UnityEngine.UI.LoopScrollDataSource.New()  --Quotes
...
function WalletCtrl:Active()
    ...
    Event.AddListener("ValidationPhoneCode",self.ValidationPhoneCode,self) --Verification verification code callback
...
function WalletCtrl:Hide()
    ...
    Event.RemoveListener("ValidationPhoneCode",self.ValidationPhoneCode,self) --Verification verification code callback
...
function WalletCtrl:_getComponent(go)
    ...
    --WithoutWalletContent   Create a wallet
    ...
    --DeclarationContent  User Agreement
    ...
    --RechargeAmountContent  Enter the recharge amount (newly added)
    ...
    --detailsRoot  Withdrawal details
    ...
    --phoneRoot   Withdrawal verification
    ...
    --PasswordContent   Payment password
...
--Initialize when opening the wallet
...
--User agreement page OK button opens to set password
function WalletCtrl:_clickAgreeBtn(ins)
    PlayMusEff(1002)
    ins:openPaymentPassword()
end
--Set password page confirmation
...
--Open the password box (newly added)
...
--Close password popup (newly added)
...
---Request withdrawal and get verification code
...
--Click NEXT to send verification code
...
--Detect and save the amount you want to withdraw
...
--Check the wallet address saved input
function WalletCtrl:saveEthAddr()
    self.ethAddr = self.addressInput.text
end
--Detect and save the entered mobile phone verification code
...
function WalletCtrl:openUserAgreement()
    ...
    --Temporarily open the user agreement by default and click next (you may need to change it to read before you can click OK)
...
--Open the wallet recharge amount (newly added)
...
--Close the wallet recharge amount (newly added)
...
function WalletCtrl:openQRCode(data)
    ...
    --self.QRCodeImg
    local currentTime = TimeSynchronized.GetTheCurrentServerTime() --millisecond
    ...
    self.moneyPurchaseId = data.RechargeRequestRes.PurchaseId   --Order Id
    self.moneyEthAddr = data.RechargeRequestRes.EthAddr   --address
...
--Close the withdrawal details and open the phone verification
...
-------------------------------------------------- -------------------Callback----------------------------- ----------------------------------------------
--Callback for successful wallet creation
...
--Recharge request successful callback
...
--Successful withdrawal order request
function WalletCtrl:reqDisChargeOrderSucceed(data,passWard)
    if data ~= nil then
        ---Package a method, open the phone verification ---
        self:openPhoneCode()
        -------------------------------------------------------------
...
--Verification verification code callback
...
--Parse payment password and private key
...
--Refresh time to get verification code
