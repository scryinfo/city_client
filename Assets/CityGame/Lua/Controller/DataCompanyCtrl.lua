...
function DataCompanyCtrl:Awake()
    ...
    self:initData()
    myOwnerID = DataManager.GetMyOwnerID()      --Own unique id
end
...
function DataCompanyCtrl:_receivePromoteCompanyDetailInfo(detailData)
    ...
    if self.groupMgr == nil then
        if detailData.info.state == "OPERATE" then -- in operation
    ...
    DataCompanyPanel.openBusinessItem:initData(detailData.info,BuildingType.Municipal)      --Open
    --History test
    UnitTest.Exec_now("abel_0426_AbilityHistory", "e_AbilityHistory",self.m_data.insId)
    --People traffic signing list test
