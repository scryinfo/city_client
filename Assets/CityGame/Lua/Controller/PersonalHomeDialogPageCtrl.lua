...
---Personal homepage popup
PersonalHomeDialogPageCtrl = class('PersonalHomeDialogPageCtrl',UIPanel)
UIPanel:ResgisterOpen(PersonalHomeDialogPageCtrl) --Open the registration method
...
function PersonalHomeDialogPageCtrl:_getComponent(go)
    ...
    self.friendSendMessageBtn = trans:Find("root/otherOpen/friends/sendMessageBtn")  --If you are a friend, you can only chat, no more friends
...
function PersonalHomeDialogPageCtrl:_initData()
    ...
    if self.m_data.id ~= DataManager.GetMyOwnerID() then
        ...
        if self.m_data.des == nil or self.m_data.des == "" then
            tempStr = GetLanguage(17020007)  --Defaults
        end
        ...
        if self.m_data.des == nil or self.m_data.des == "" then
            tempStr = GetLanguage(17010004)  --Defaults
        end
    ...
    self.nameText.rectTransform.sizeDelta = Vector2.New(self.nameText.preferredWidth + 50, self.nameText.rectTransform.sizeDelta.y)  --Add the width of a sex picture
    self.companyText.text = self.m_data.companyName
end
--Refresh the player's name
