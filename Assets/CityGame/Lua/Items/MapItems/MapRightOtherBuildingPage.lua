...
---Click on other people's buildings
...
function MapRightOtherBuildingPage:initialize(viewRect)
    ...
    self.notOpenTranInfo = trans:Find("bottomRoot/notOpenTran") -- The building is not open
    self.notOpenText = trans:Find("bottomRoot/notOpenTran/Text"):GetComponent("Text") -- The building is not open

    self.simpleInfo = trans:Find("bottomRoot/simpleInfo")  --Other buildings that only display simple information
    ...
    self.promotionInfo = trans:Find("bottomRoot/promotionInfo")  --Promotion company
    self.promotionItem = MapRightOtherPromotePage:new(self.promotionInfo)

    self.technologyInfo = trans:Find("bottomRoot/technologyInfo")  --Research Institute
    self.technologyItem = MapRightOtherTechnologyPage:new(self.technologyInfo)

    self.matGoodInfo = trans:Find("bottomRoot/matGoodInfo")  --Show the buildings of Mat Good
...
--Detailed data & currently searched data
...
--Show details based on search type
...
function MapRightOtherBuildingPage:showByType(typeData)
    ...
    if typeData.detailId == nil then
        ...
        elseif typeData.typeId == EMapSearchType.Builds then  --Building type search
...
function MapRightOtherBuildingPage:_createInfoByType(buildingType)
    if buildingType == BuildingType.House or buildingType == BuildingType.RetailShop then  --Sign a contract
        self:_createSign()
...
function MapRightOtherBuildingPage:_createSign()
    ...
    local data1 = {infoTypeStr = "SignTime", value = str1}  --Signing time
    self.items[#self.items + 1] = self:_createShowItem(data1, self.simpleShowRoot)

    local str3 = string.format("%0.2f", self.data.lift * 100).."%"
    local data3 = {infoTypeStr = "SignPromotion", value = str3}  --Signing bonus
...
function MapRightOtherBuildingPage:_createWarehouse()
    ...
    local data1 = {infoTypeStr = "WarehouseTime", value = str1}  --time
    self.items[#self.items + 1] = self:_createShowItem(data1, self.simpleShowRoot)

    local str3 = self.data.availableSize
    local data3 = {infoTypeStr = "WarehouseRent", value = str3}  --Available positions
    self.items[#self.items + 1] = self:_createShowItem(data3, self.simpleShowRoot)
end
--
