...
function CompanyWaterItem:initialize(viewRect, goodsDataInfo,isIncome)
    ...
    self.freight = self.viewRect:Find("freight"):GetComponent("Text")   --Freight
    ...
    if isIncome then
        ...
        if goodsDataInfo.itemId == 2000 then   --Citizen check-in
        ...
        elseif goodsDataInfo.itemId == 3000 then  --Land for sale
            self.goodsBg.localScale = Vector3.zero
            self.bg.localScale = Vector3.one
        ...
        else   --Commodity raw materials
            self.goodsBg.localScale = Vector3.one
            self.bg.localScale = Vector3.zero
            --if math.floor(goodsDataInfo.itemId / 100000) == 21 then  --raw material
            --    LoadSprite(Material[goodsDataInfo.itemId].img, self.goods)
            --elseif math.floor(goodsDataInfo.itemId / 100000) == 22 then --commodity
        ...
        if goodsDataInfo.itemId == 1000 then --salary giving
            self.goodsBg.localScale = Vector3.zero
            self.bg.localScale = Vector3.one
        ...
        elseif goodsDataInfo.itemId == 3000 then  --Land purchase
            self.goodsBg.localScale = Vector3.zero
            self.bg.localScale = Vector3.one
            LoadSprite("Assets/CityGame/Resources/Atlas/Company/water/icon-land.png", self.icon,true)
            self.name.text = GetLanguage(11010013)
            self.sizeName.text = GetLanguage(18030003) .. goodsDataInfo.name
        elseif goodsDataInfo.itemId == 4000 then  --Land auction
            self.goodsBg.localScale = Vector3.zero
            self.bg.localScale = Vector3.one
            LoadSprite("Assets/CityGame/Resources/Atlas/Company/water/icon-land.png", self.icon,true)
            self.name.text = GetLanguage(20010007)
            self.sizeName.text = GetLanguage(18030003) .. goodsDataInfo.name
        elseif goodsDataInfo.itemId == 5000 then  --Eva point purchase
            self.goodsBg.localScale = Vector3.zero
            self.bg.localScale = Vector3.one
